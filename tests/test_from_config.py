...
import json
import base64
from sqlalchemy import create_engine, MetaData, Table, Column, Integer, String, ForeignKey, DateTime, Date, Boolean, Float, Text, LargeBinary, CheckConstraint, UniqueConstraint, PrimaryKeyConstraint, ForeignKeyConstraint, Computed
...
class TestFromConfigExtensive:
    def setup_users_and_posts(self, db_path):
        engine = create_engine(f'sqlite:///{db_path}', poolclass=NullPool)
        metadata = MetaData()
        Table('users', metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('name', String, nullable=False), Column('email', String, nullable=False, unique=True))
        Table('posts', metadata, Column('post_id', Integer, primary_key=True, autoincrement=True), Column('user_id', Integer), Column('content', String))
        metadata.create_all(engine)
        return engine
    ...
    def test_table_with_composite_pk(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('composite', metadata, Column('a', Integer), Column('b', Integer), PrimaryKeyConstraint('a', 'b'))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'composite', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_get_empty_table(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('emptytable', metadata, Column('id', Integer, primary_key=True), Column('foo', String))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'emptytable', 'crud': ['get']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_unique_constraint_violation(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('uniq', metadata, Column('id', Integer, primary_key=True), Column('foo', String, unique=True))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'uniq', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_patch_put_delete_on_nonexistent_row(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('testpk', metadata, Column('id', Integer, primary_key=True), Column('foo', String))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'testpk', 'crud': ['get', 'put', 'patch', 'delete']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_complex_table_types_and_defaults(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('complex', metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('name', String, nullable=False), Column('age', Integer, server_default=text('30')), Column('score', Float), Column('is_active', Boolean, server_default=text('1')), Column('created_at', DateTime, server_default=text('CURRENT_TIMESTAMP')), Column('updated_at', DateTime), Column('notes', String), Column('extra', String, server_default=text('foo')), UniqueConstraint('name'))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            resp = await client.post('/complex/', json={
                'name': 'Beta', 'age': 40, 'score': 99.5, 'is_active': 0,
                'created_at': '2022-01-01', 'updated_at': now, 'notes': 'test', 'extra': 'custom'
            })
            if resp.status != 201:
    ...
    async def test_foreign_key_relationships(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('authors', metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('name', String, nullable=False, unique=True))
        Table('books', metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('author_id', Integer, ForeignKey('authors.id'), nullable=False), Column('title', String, nullable=False), ForeignKeyConstraint(['author_id'], ['authors.id'], ondelete='CASCADE'))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
    ...
    async def test_self_referential_fk_tree(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('categories', metadata, Column('id', Integer, primary_key=True, autoincrement=True), Column('name', String, nullable=False), Column('parent_id', Integer, ForeignKey('categories.id', ondelete='CASCADE')))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'categories', 'crud': ['get', 'post', 'patch']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_check_constraint(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('scores', metadata, Column('id', Integer, primary_key=True), Column('value', Integer, CheckConstraint('value >= 0 AND value <= 100')))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'scores', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_default_expression_timestamp(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('logs', metadata, Column('id', Integer, primary_key=True), Column('message', String), Column('created_at', DateTime, server_default=text('CURRENT_TIMESTAMP')))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'logs', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_multiple_unique_constraints(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('people', metadata, Column('id', Integer, primary_key=True), Column('email', String), Column('username', String), UniqueConstraint('email'), UniqueConstraint('username'))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'people', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_nullable_and_not_null_columns(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('things', metadata, Column('id', Integer, primary_key=True), Column('required', String, nullable=False), Column('optional', String), Column('with_default', String, server_default=text('foo')))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'things', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            # All fields
            resp = await client.post('/things/', json={'id': 2, 'required': 'ok', 'optional': 'maybe', 'with_default': 'bar'})
            assert resp.status == 201
            thing = await resp.json()
    ...
    async def test_json_column(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('docs', metadata, Column('id', Integer, primary_key=True), Column('data', String))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'docs', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_multilevel_fk_cascade(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('grandparent', metadata, Column('id', Integer, primary_key=True), Column('name', String))
        Table('parent', metadata,
              Column('id', Integer, primary_key=True),
              Column('grandparent_id', Integer, ForeignKey('grandparent.id', ondelete='CASCADE')),
              Column('name', String),
    ...
    async def test_partial_unique_constraint(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('emails', metadata, Column('id', Integer, primary_key=True), Column('email', String, unique=True))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'emails', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_generated_virtual_column(self, temp_db, make_config):
        ...
        metadata = MetaData()
        try:
            Table('gen', metadata, Column('id', Integer, primary_key=True), Column('a', Integer), Column('b', Integer), Column('sum', Integer, Computed('a + b')))
            metadata.create_all(engine)
    ...
    async def test_trigger_audit_log(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('main', metadata, Column('id', Integer, primary_key=True), Column('val', String))
        Table('audit', metadata, Column('id', Integer, primary_key=True), Column('main_id', Integer, ForeignKey('main.id')), Column('action', String))
        metadata.create_all(engine)
    ...
    async def test_many_to_many_relationship(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('students', metadata, Column('id', Integer, primary_key=True), Column('name', String))
        Table('courses', metadata, Column('id', Integer, primary_key=True), Column('title', String))
        Table('enrollments', metadata, Column('student_id', Integer), Column('course_id', Integer), PrimaryKeyConstraint('student_id', 'course_id'), ForeignKeyConstraint(['student_id'], ['students.id']), ForeignKeyConstraint(['course_id'], ['courses.id']))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
    ...
    async def test_composite_foreign_key(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('refd', metadata, Column('a', Integer), Column('b', Integer), PrimaryKeyConstraint('a', 'b'))
        Table('ref', metadata, Column('id', Integer, primary_key=True), Column('a', Integer), Column('b', Integer), ForeignKeyConstraint(['a', 'b'], ['refd.a', 'refd.b']))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
    ...
    async def test_enum_like_check_constraint(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('pets', metadata, Column('id', Integer, primary_key=True), Column('type', String, CheckConstraint('type IN (\'dog\',\'cat\',\'bird\')')))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'pets', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_large_text_blob_columns(self, temp_db, make_config):
        engine = create_engine(f'sqlite:///{temp_db}', poolclass=NullPool)
        metadata = MetaData()
        Table('files', metadata, Column('id', Integer, primary_key=True), Column('name', String), Column('data', LargeBinary))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'files', 'crud': ['get', 'post']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_multiple_datetime_columns(self, temp_db, make_config):
        ...
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
                {'name': 'events', 'crud': ['get', 'post', 'patch']}
            ]
        }
        config_path = make_config(config)
        os.environ['DATABASE_URL'] = f'sqlite:///{temp_db}'
    ...
    async def test_multiple_fks_to_same_table(self, temp_db, make_config):
        ...
        Table('tasks', metadata, Column('id', Integer, primary_key=True), Column('user_id', Integer), Column('manager_id', Integer), ForeignKeyConstraint(['user_id'], ['users.id']), ForeignKeyConstraint(['manager_id'], ['users.id']))
        metadata.create_all(engine)
        config = {
            'database_url': f'sqlite:///{temp_db}',
            'tables': [
...
class TestFromConfig:
    ...
    async def test_reflect_and_crud(self, temp_db_and_config):
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            # PUT (update)
            resp = await client.put(f'/users/{user_id}', json={'name': 'Alice2', 'email': 'alice2@example.com'})
            assert resp.status == 200
            user = await resp.json()
